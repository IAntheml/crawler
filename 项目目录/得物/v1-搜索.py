...
def get_x_auth_token(uid):
    ...
    param_dict = {"loginToken": "", "platform": "android",
                  "timestamp": ctime, "uuid": uid, "v": "4.74.5"}
...
def search(key, uid, x_auth_token):
    ctime = str(int(time.time() * 1000))
    param_dict = {
        "hideAddProduct": 0,
        "title": key,
        ...
        "abTest":
            json.dumps([{"name": "search_unequlheight_spu_strategy", "value": "0"}], separators=(',', ':')),
        "sortMode": 0,
        "typeId": 0,
        "sortType": 0,
        "catId": 0,
